...
const Create = ({ allCategories, errorCategories }) => {
  ...
  const renderCheckbox = category => {
    ...
    return (
      <div key={category.id}>
        ...
        <input type="checkbox" checked={isChecked} onChange={handleCheckboxChange} name="categories" id={category.id} />
      </div>
    );
  };
